...
public class PdfFontValidatorV2
{
    ...
    public IEnumerable<PdfValidationResultV2> ValidateFonts(PdfPages pdfPages)
    {
        ...
        IEnumerable<PdfFontDescriptorV2> scanResult = ScanForFonts(pdfPages);
        IEnumerable<PdfFontDescriptorV2> validationResult = scanResult.Where(f => !f.Embedded && !IsStandardFont(f.FontName.ToString()));

        foreach (PdfFontDescriptorV2 item in validationResult)
        {
